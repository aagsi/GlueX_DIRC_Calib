...
// for survay study calculate spr for DIRC wall
#include "TStopwatch.h"
TStopwatch timer;

int analyses2_1(TString infile="out2.root"){// survay.root.root //out2.root
    ...
    TH1F* histo_trck_spr[42][26];
    for(Int_t i=0; i<42; i++) {
        for(Int_t j=0; j<26; j++){
            histo_trck_spr[i][j]= new TH1F(Form("histo_trck_spr_%d_%d",i,j),Form("xbin %d nbar %d ; SPR [mrad]; Entries [#]",i,j) ,100,-20,20);
        }
    }
    ...
    TH2F * ratio_sigma_average = new TH2F( "ratio_sigma_average" , "; Bar Hit X [cm]; Bar number [#]", 40, 0, 40, 24, 0, 24);
        ...
        TH2F * histo_test = new TH2F( "histo_test" , "; Bar Hit X [cm]; Bar number [#]", 40, 0, 40, 24, 0, 24);
    ...
    for (Long64_t i=0;i<nentries;i++) {
        ...
        histo_trck_spr[x_pos_tmp_bin][track_nbar]->Fill(track_spr*1000);
    ...
    TF1 *fit_gause3 = new TF1("fit_gause3","[0]*exp(-0.5*((x-[1])/[2])*(x-[1])/[2])",-20,20);
    fit_gause3->SetLineColor(kBlack);
    fit_gause3->SetParameters(100,9,2);
    fit_gause3->SetParNames("p0","mean ","sigma");
    fit_gause3->SetParLimits(0,0.1,1E6);
    fit_gause3->SetParLimits(1,5,12);
    fit_gause3->SetParLimits(2,1,10);
    ...
    if(true){
        ...
        for(Int_t i=0; i<42; i++) {
            for(Int_t j=0; j<26; j++){
                ...
                if(histo_trck_spr[i][j]->GetEntries()<500)continue; // 500
                histo_trck_spr[i][j]->Fit("fit_gause3","M","", -20, 20);
                double trck_mean_fit = fit_gause3->GetParameter(1);
                double trck_sigma_fit = fit_gause3->GetParameter(2);
                //shift_array_detailed[i][j]=fit_gause1->GetParameter(1);
                if(histo_trck_spr[i][j]->GetEntries()<200){
                    trck_mean_fit = -1000;
                    trck_sigma_fit = 0;
...
//                cc5->cd();
//                cc5->Update();
//                histo_trck_spr[i][j]->Draw();
//                cc5->Update();
//                TLine *lineMeanSHift= new TLine(0,0,0,1000);
//                lineMeanSHift->SetX1(fit_gause3->GetParameter(1));
//                lineMeanSHift->SetX2(fit_gause3->GetParameter(1));
//                lineMeanSHift->SetY1(gPad->GetUymin());
//                lineMeanSHift->SetY2(gPad->GetUymax());
//                lineMeanSHift->SetLineColor(kRed);
//                lineMeanSHift->Draw();
...
//                cc5->WaitPrimitive();
    ...
    ratio_mean_average->GetXaxis()->SetTitle(" ");
    ratio_mean_average->GetYaxis()->SetTitle(" ");
    ratio_mean_average->SetTitle("Mean of SPR per track  ");
    ratio_mean_average->Divide(histo_pos_xy_TrkMeanShift_occu_average);
    ratio_mean_average->SetMinimum(0);
    ratio_mean_average->SetMaximum(14);
    ratio_mean_average->Draw("COLZ");
    ...
    ratio_sigma_average->GetXaxis()->SetTitle(" ");
    ratio_sigma_average->GetYaxis()->SetTitle(" ");
    ratio_sigma_average->SetTitle("Sigma of SPR per track ");
    ratio_sigma_average->Divide(histo_pos_xy_TrkSigmaShift_occu_average);
...
//                
//                content1 = ratio_mean_average->GetBinContent(i,j);
//                content2 = ratio_sigma_average->GetBinContent(i,j);
//                content3 = sqrt(content1*content1+content2*content2);
//                histo_test->SetBinContent(i,j,content3);
//            }
//        }
//    
//    glx_canvasAdd("r_test",800,400);
//    histo_test->SetMinimum(0);
//    histo_test->SetMaximum(14);
//    histo_test->Draw("COLZ");

...
void DSelector_justin_1_analyzer::Init(TTree *locTree)
{
    ...
    dHist_KsMass_Measured = new TH1I("KsMass_Measured", ";#pi^{#plus}#pi^{#minus} Invariant Mass", 250, 0.36, 0.635);
    dHist_KsMass_KinFit = new TH1I("KsMass_KinFit", ";#pi^{#plus}#pi^{#minus} Invariant Mass", 250, 0.36, 0.635);
...
Bool_t DSelector_justin_1_analyzer::Process(Long64_t locEntry)
{
    ...
    for(UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i)
    {
        ...
        /**************************************** ks VerteX Decay CUT ACTION ************************************************/
        //dHist_StepVertexZ->Fill(locKsP4.Z());
        //dHist_StepVertexYVsX->Fill(locKsP4.X(), locKsP4.Y());

        // x = locParticleComboStepWrapper
        // y = dParticleComboWrapper
        // z = DKinematicData
        TLorentzVector locStepSpacetimeVertex = dStep1Wrapper->Get_X4();
        int locFromStepIndex = dStep1Wrapper->Get_InitDecayFromIndices().first;
        ...
        dHist_DetachedLifetime->Fill(locDeltaSpacetime.T());
        //dHist_DetachedPathLength->Fill(locPathLength);
        dHist_DetachedPathLengthSignificance->Fill(locPathLength);//dDecayingKShortWrapper->Get_PathLengthSigma());
